...
The Data Library can run on a single server/workstation, provided it has
...
are general recommendations. If you have questions about the server you intend to use, please contact IRIDL Help <help@iri.columbia.edu>.
...
should double that to 128GB. The more memory you can install,the better the 
performance you will have. For most cases, 128GB will be more than enough.

## Storage

You _can_ put everything on one disk if necessary, but we recommend
mirroring across 2 disks in case of a single disk failure. For best
results, use fast disks such as SSDs or NVMe for the operating
system. These can be as small as 128GB for the OS. You should try to
get Enterprise hard disk drives. They are generally more reliable and
last significantly longer.

You will also need space for your data. If you're not using the OS
disk for data storage, we recommend getting multiple disks so you can
mirror the drive in case of a single disk failure. A mirror is not the
same as a backup--see the following section.

The amount of disk space you may need is unique to your
installation. We suggest getting disks that are about twice the
current need.

### Backups

Even if you are using mirrored disk drives, it is important to have
regular backups to an external disk or network volume to protect
against data loss from things like human error or ransomware.  You
should back up the /data and /home directories.  If you do this
regularly, you can ensure that your system will be quickly recoverable
in case of disaster.
